...
Plusieurs modèles supervisés ont été développés, notamment la régression logistique, les machines à vecteurs de support (SVM linéaire et radial), les arbres de décision (CART), le k plus proches voisins (KNN), les forêts aléatoires (Random Forest) et le Boosting (XGBoost). L'ensemble des modèles ont été optimisés par validation croisée, en ajustant leurs hyperparamètres spécifiques et en utilisant des techniques de sur-échantillonnage comme SMOTE-NC pour corriger le déséquilibre des classes.
